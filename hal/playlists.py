...
class indexer:
    ...
    def root_oneclick(self):
        try:
            regex.clear()
            url = 'https://cmanbuilds.com/NEW%20BLACK%20LIGHTING/1click/cats/qselect.xml'
            self.list = self.playlists_list(url)
...
class resolver:
    ...
    def get(self, url):
        try:
            ...
            items = [self.extract_label(i[1]) if self.extract_label(i[1]) is not None else i for i in items]
